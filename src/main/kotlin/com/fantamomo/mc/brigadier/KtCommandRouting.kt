...
abstract class KtRoutingKey<S, T>(
     ...
     */
    abstract fun fallbackValue(context: CommandContext<S>): T

    private class Impl<S, T>(name: String, cacheValue: Boolean, val fallbackProvider: (CommandContext<S>) -> T) :
        KtRoutingKey<S, T>(name, cacheValue) {
        override fun fallbackValue(context: CommandContext<S>): T = fallbackProvider(context)
    }

    private class Static<S, T>(name: String, val value: T) : KtRoutingKey<S, T>(name, true) {
        override fun fallbackValue(context: CommandContext<S>): T = value
    }
    ...
    companion object {
        ...
        ): KtRoutingKey<S, T> = Impl(name, cacheValue, fallbackProvider)
        ...
        fun <S, T> static(
            name: String,
            value: T
        ): KtRoutingKey<S, T> = Static(name, value)
    }
}
